import { countFullyContainingSets, countOverlappingSets } from './advent.004';
...
describe('Advent 04', () => {
  ...
  describe('countOverlappingSets', () => {
    describe('On an empty file', () => {
      it('should return 0', async () => {
        await expect(
          countOverlappingSets({
            inputFilePath: `${__dirname}/test-data/empty.txt`,
          })
        ).resolves.toBe(0);
      });
    ...
    describe('On a file with all sections', () => {
      it('should return a positive value', async () => {
        ...
        ).resolves.toMatchInlineSnapshot(`804`);
      });
    });
  });
});
